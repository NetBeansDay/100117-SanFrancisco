...
I deal mostly with tools for developers. Both development tools and infrastructure tools for developers. This session is going to share some of the lessons we learned when we build our own tools. Those are lessons, actually, from our own development teams that are building products. Specifically those are lessons around how to do agile team development and a little bit of DevOps as well, using a Cloud environment. 
...
One thing to highlight here is the fact that our development team there those 170 people, they are spread all over the world. We have some of them sitting in the Oracle HQ, some of them are sitting in the Czech Republic, some of them are in the UK, some of them are in India. Some of them just work from home in various places. All of them are collaborating, checking in code and stuff like that and then the build basically gets this unified product out of it.

Also, every two weeks we take the master branch and we do a build on it. That basically creates the product, deploys it, runs automated tests, and also allows our QA guys to do manual testing on this. This is kind of our big two week end of sprint build, which basically is supposed to create our candidates for product release. Not that we're every two weeks we are releasing, sometimes it's more like once a month or something like that. But this is actually how we produce the product, and at the end of the day you will get access to our Cloud.


20:00 --------------------------------------

So, some tips from our experience about builds. You want some notification going out from your build. It's kind of like you know like the the question of this tree falls in the forest and nobody hills it is actually happen if the bill fails and nobody knows about it it doesn't mean anything so when a bill fails we send notification to all the team to know okay this happened and it also prevents the idea of all I didn't know it was broken things like that situation and you need to require a test like we require a test as part of the build so every feature that is being big okay we're big believer in test driven development and when the developer produces the new features he built the test into it and we automate it and include it as part of the build for this feature and the fact that we do we will have bills help us to mostly like faster realize it dial problems okay this twice they build across the product helps us identify programs much sooner and when you identify a problem sooner in the development cycles it's usually easier to detect what went wrong because it's something that's happened in the last like half days okay and it is really usually to fix it but another thing and there is no such thing as build / zone okay what you need to remember here is the build is automated there's no


 person that actually works and out some things okay so you shouldn't be worried about having the computer do a lot of work for you okay so doing those two builds a day or doing those little deals on every commit don't worry about the fact that you're overloading the system this is going to hurt you at the end of the day I know that some people say okay we'll do one bill the day that that's not the solution like when you need it Rannoch build in our case for example and we actually have a situation where there's almost no part of the day where our build servers are not doing some we are not afraid to over you and another thing by the way that we are doing with build automation it's not just part of the release of the product we're actually using the build automation to monitor their live application so we have watchdog bills that basically do things like check the performance and pink pay you the URL and make sure that everything works okay and those are against how to follow alternation so this was a little bit about how we do agile and how we do continuous integration and delivery I want to take you to the typical day of the various roles in our organization so again I'll do it with slides and then I'll actually show you how it looks so I'm going to talk about one person who's the developer ok so what the developer usually do the way that their day starts is that be getting emails about the new a task Oh feature the day are required to build or bad that they need to fix em and this is how they know what to do ok it's not every day that I get an email but it's usually when if something is assigned to them by the way if there is a situation where nothing is assigned to you today it doesn't mean that you can go home what it means is that we have a list of open issues with no assignment you go over there and you pick one that you know that you were able to work with and this is the one that you're going to work so you know the test that you're supposed to do today then you branch the code and you'll start working on your coat and by the way walking on the code the total you can use any idea you want ok so i can for example has an ID called jdeveloper samo fellow developers use this some people use the clip some people use netbeans some people using telly j some people actually call directly in the cloud so if you being at the keynote in the morning they showed you this case we actually have a code base of coding as environments in the cloud so formula browser you can call each one whatever suits him okay at the end of the day all of those coding environments are connected to our gate repository so once you're done coding in your favorite environment you can actually commit and put the changes into the digital and this files a build on your specific branch okay and if everything goes perfectly fine and you're ready with your code it past your yo test you go and you do a male's request mel's request initiate also a code review cycles and you do kind of iterative reviewing of the code you get comments every comment gets you you get an email about it and then you modify your code again run the bills automatically when everything is done and your code is the pool you merge it into the mast cells and then you close the task Val kisses resolved okay and then you start all from the beginning pick up the next up and do this so this is what a developer does and the DevOps guys we actually don't have a lot of them ok because DevOps all they do is like most of our DevOps is automatic right so melty girl a build a unit tests are executed automatically our build produce a binaries that go into a maven repository will remain and roll our binaries it's a bill fails you get a notification and the scheduled bills that one every like a couple of times a day em when a bill is successful we notify a QA automatically a QA then goes in and can verify the things that will master's touch that were closed and actually closed the updates anybody that they are now closed after death we ran selenium UI testing automatically and I will develop guys what mostly what they do is okay they define the initial build files okay all the build processes but then what most of the time is spent on is actually monitoring designing the environment okay so they'll ops is instead of focusing on kind of the wasteful path of trying to take the code and move it along the chain it's actually they are focused on monitoring the tower ranfeng platform is performing well in stable stuff like that which is what we want your ox guys to do you want to make sure that your Lantern platform your finished product is actually successful let's talk about the development manager and what is they look like so one thing the development manager usually do in the morning is look at the Activity Stream basically look at what has happened during the night because again as I said there are team all over the world working on it in different time zones so he looks at what has been happening and what new issues were assaulted stuff like that and then he goes over than he does is shoe management so he looks at the issues he prioritized them he assigns them to people he can click the status of tissue stuff like that the next thing that he might be doing is look at Ramez request which are also code review cycles and some of our managers are also acting as the sprint manager so they actually track the 

progress of the sprint look at various with both and how are we matching our goals stuff like that and at the end of the day they are also the ones who are working on this release branch that is the protected branch that will go over and become our product they are the only ones who are approved to approve actually merges into that one so they also do that 
...
Everything is in the cloud is so much simpler for us. It used to take us months to get an environment for building because we needed to buy the servers and the software and install everything and connect everything. Now the team at Oracle can get this full environment in a matter of two minutes. They put in a request, everything is provisioned for them in the Cloud, and they can start working.

By the way you can do the same thing. 

And then team collaboration is key for us. Here are some statistics about what we do, again, 40 Git repositories, 170 project members, 50 of them are actual coders 200 commits a week, 50 builds a day, over 20k of issues that are documented in our system and 250 Wiki pages that we use to share information among our team.

What it gives us is full traceability. We can collect something from the issue being reported, to looking at how long it takes us to fix it, all the way to the source code changes, the bill that will file the deploy everything is placeable okay it gives us a faster feedback on our code commit because we get in immediate build and build success and test results it improves our collaboration you so how is it is to request the code review from someone sharing the knowledge of the experts with the beginners stuff like that and we have one stop shop and for agile and developed accessibility its cloud-based you can go anywhere like I'm feeling the d hotel doing the same stuff that people in the office do then if you're on a vacation and you have a you checked in something that broke the bills there's no excuses can still access the code and fix it to our cloud right so even if you don't have your laptop with you and and the guest is the provisioning and scaling so if you want to charge it on your own and you got a location for getting piles environment or actually getting the cloud environment for the author cloud we actually provision this product what you saw today this whole thing is available for you for free with almost every other pastors that you'll get if you're getting the Java cloud service of our application container crowd all our database cloud service so our container docker containers out service all of them come with an instance of developer cloud service for free and you can use it and try it out and see how it can help you manage your own project doing the same thing I just did you everything I showed you by the way is on our current production version nothing future everything is been out of a while so you can just try it on your own we also have a bunch of tutorials and videos and blogs phone for question if you go to this website and all the information is there and that's all the time i had for today if you have any questions because i need to let everyone go hey i'll be here just come back and ask your questions and i'll be happy to answer them thank you very much.
